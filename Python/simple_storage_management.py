...
def add_element(database, product, quantity):
    ...
    if (index >= 0):
        # Transform item into list to allow for modifications
        database[index] = list(database[index])
        database[index][1] = database[index][1] + \
            quantity  # Modify the quantity
        # Return item to tuple
        database[index] = tuple(database[index])
    else:
        # If item not found, add item to database
        database.append((product, quantity))
...
def remove_element(database, product, quantity):
    ...
    # Index of product not found, see find_element function
    if index == -1:
        print(f"Product {product} not found.")
        return database
    # Index of found product and the product can be removed
    elif (index >= 0) and (database[index][1] >= quantity):

        # Transform tuple into list to modify quantity
        database[index] = list(database[index])
        database[index][1] = database[index][1] - \
            quantity                        # Modify quantity
        # Return tuple to list
        database[index] = tuple(database[index])

        print(f"The quantity {
              quantity} was removed from the product {product}.")

        # Case that remains product in database
        if database[index][1] > quantity:

            print(f"It remains {database[index][1]} of the product {product}")
            return database

        # Case that no product remains in database
        elif database[index][1] == 0:
...
def replace_element(database, product_old, product_new, quantity):
    ...
    if index >= 0:                                      # Check if item is found
        # Remove old item from database
        database.pop(index)
        # Insert nem item into database
        database.insert(index, (product_new, quantity))
